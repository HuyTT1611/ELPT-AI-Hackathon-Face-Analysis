...
def main(cfg, args):
    ...
    timestamp = time_str()
    stdout_file = os.path.join(log_dir, f'hackathon_log.txt')
    ...
    valid_set = FaceAttrHackathon(cfg=cfg, csv_file=cfg.DATASET.PHASE2_ROOT_PATH + '/valid.csv', transform=valid_tsfm,
...
def trainer(cfg, args, epoch, model, model_ema, train_loader, valid_loader, criterion, optimizer, lr_scheduler,
    ...
    for e in range(epoch):
        ...
        if cfg.METRIC.TYPE == 'face':

            train_result = get_face_metrics(train_gt, train_probs, index=None, cfg=cfg)
            valid_result = get_face_metrics(valid_gt, valid_probs, index=None, cfg=cfg)
            ...
            if args.local_rank == 0:
                tb_visualizer_face(tb_writer, lr, e, train_loss, valid_loss, train_result, valid_result,
