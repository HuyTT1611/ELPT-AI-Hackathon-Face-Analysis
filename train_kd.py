...
from utils_dkd import colorstr, Save_Checkpoint, DirectNormLoss, KDLoss
...
def main(args, teacher, T_EMB):
    ...
    if args.multiprocessing_distributed:
        ...
        main_worker(args.gpu, ngpus_per_node, args, teacher,T_EMB)
...
def train(model, teacher, T_EMB, train_loader, optimizer, criterion, kd_loss, nd_loss, ngpus_per_node, args, epoch):
    ...
    for step, (images, labels) in enumerate(train_loader):
        ...
        with torch.no_grad():
            t_emb, t_logits = teacher(images)
        
        # cls loss
        cls_loss = criterion(s_logits, labels) * args.cls_loss_factor
        # KL loss
        div_loss = kd_loss(s_logits, t_logits) * min(1.0, epoch/args.warm_up)
        # direction and center loss
        norm_loss = nd_loss(s_emb=s_emb, t_emb=t_emb, T_EMB=T_EMB, labels=labels)
        ...
        s2 = ' - {:.2f}ms/step - norm_loss: {:.3f} - div_loss: {:.3f} - cls_loss: {:.3f} - train_loss: {:.3f} - train_acc_top1: {:.3f} - train_acc_top5: {:.3f}'.format(
...
if __name__ == "__main__":
    ...
    parser.add_argument('--world_size', default=2, type=int, help='number of nodes for distributed training')
    ...
    # ViT-S    ./ckpt/vits/center_emb_train.json
    # ViT-B    ./ckpt/vitb/center_emb_train.json
