...
input_feature = 1280 #efficientnet
#input_feature = 3072 # nfnets

class BaseClassifier_LW(nn.Module):
    def __init__(self, nattr):
        super().__init__()
        self.logits = nn.Sequential(
            nn.Linear(input_feature, nattr),
    ...
    def forward(self, feature):
        ...
        # feat = self.avg_pool(feature).squeeze(-1).squeeze(-1)
        feat = self.avg_pool(feature).flatten(start_dim=1)
        #print(feat)
        x = self.logits(feat)
        return x
...
class FeatClassifier_LW(nn.Module):

    def __init__(self, backbone, classifier):
        super(FeatClassifier_LW, self).__init__()
